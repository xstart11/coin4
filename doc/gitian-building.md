...
*Setup instructions for a gitian build of MNDX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MNDX
...
Clone the git repositories for mndx and gitian and then checkout the mndx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mndx-crypto/mndx.git
cd mndx
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mndx repository
...
To build MNDX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mndx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mndx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mndx-crypto/mndx
...
For example:
```bash
URL=https://github.com/mndxproject/mndx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mndx=${COMMIT} --url mndx=${URL} ../mndx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mndx=${COMMIT} --url mndx=${URL} ../mndx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mndx=${COMMIT} --url mndx=${URL} ../mndx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "mndxcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mndx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mndx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mndx-build.assert
```
...
[mndx/gitian.sigs](https://github.com/mndx-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
